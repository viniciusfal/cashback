...
export function ListCredits() {
  ...
  const [cashbackConfirmed, setCashbackConfirmed] = useState<{ [key: string]: boolean }>({});
  ...
  useEffect(() => {
    // Recupera o estado do cashback confirmado do localStorage
  ...
  function handleDialogOpen(transaction) {
  ...
  const calcularCashback = (local: string, ticketPrice:number) => {
    ...
    if(local === 'SOBRADINHO'){
      if(ticketPrice >=159.5 && ticketPrice < 319) return `${simplescash} R$ 14,50`
      if (ticketPrice >= 319) return `${simplescash} R$ 29`;
    }
    if(local === 'PLANALTINA-GO') {
      if(ticketPrice >= 486.2) return `${supercash} R$ 44,20`
      if(ticketPrice >=243.1 && ticketPrice < 486.2) return `${simplescash} R$ 22,10`
    }
    return 'Nenhum Cashback Disponível';
  };

  // Função para obter detalhes do passageiro
  const getPassengerDetails = (passengerCode) => {
    ...
    <div>
      ...
      <RadioGroup defaultValue="option-one" className='flex gap-4'>
        <div className="flex items-center space-x-2">
          <RadioGroupItem value="option-one" id="option-one" onClick={() => setDoesFullVisibility(true)} />
          <Label htmlFor="option-one" className='text-sm'>Do dia</Label>
        </div>
     <div className="flex items-center space-x-2">
       <RadioGroupItem value="option-two" id="option-two"  onClick={() => setDoesFullVisibility(false)}/>
      ...
      <Table>
        ...
        <TableBody>
          {doesFullVisibility ? filteredCredits?.map((transaction) => {
            ...
            return (
              ...
              >
                ...
                <TableCell>
                  {cashbackConfirmed[transaction.id] ? (
                    ...
                    <Button
                      ...
                      variant={
                        transaction.ticketPrice < 128.7 ? 'ghost' : 'default'
                      }
                ...
                <TableCell>
                  <DialogSetCredits passengerCode={transaction.passenger_code} />
                </TableCell>
              </TableRow>
            );
